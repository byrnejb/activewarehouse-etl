...
class TransformTest < Test::Unit::TestCase
  def test_sha1_transform
    ...
    assert_equal digest_value, ETL::Transform::Sha1Transform.new(
      control, nil).transform('test', 'abc', [])
  end
  def test_block_transform
    #transforms = [Proc.new(){|name, value, row| value[0,2]}]
    #assert_equal '11', ETL::Transform::Transform.transform(
    # :ssn, '1111223333', [], transforms)
  end
  ...
  def test_string_to_date_transform
    ...
    assert_equal Date.parse('2005-01-01'), 
      t.transform(nil, '2005-01-01', [])
    assert_equal Date.parse('2004-10-20 20:30:00'), 
      t.transform(nil, '2004-10-20', [])
  end
  def test_date_to_string_transform
    ...
    t = ETL::Transform::DateToStringTransform.new(
          control, nil, {:format => '%m/%d/%Y'})
  ...
  def test_foreign_key_lookup_transform
    ...
    t = ETL::Transform::ForeignKeyLookupTransform.new(
          control, nil, configuration)
    
    assert_equal 1, t.transform(nil, 'foo', nil)
    assert_equal 2, t.transform(nil, 'bar', nil)
    assert_equal 3, t.transform(nil, 'baz', nil)
    assert_raises(ETL::ResolverError, 
      'Foreign key for bing not found and no resolver specified') do
  ...
  def test_type_transform
    control = ETL::Control::Control.parse(
      File.dirname(__FILE__) + '/control/delimited.ctl')
    assert_equal 10, ETL::Transform::TypeTransform.new(
      control, nil, {:type => :number}).transform(nil, '10', nil)
    
    assert_equal BigDecimal::ROUND_HALF_UP, 
    ...
    assert_equal '10.0000000000000000000000000000000001', 
      decimal_transformed.to_s('F')
  end
  def test_non_existent_transformer
    
  end
  def test_default_transform
    t = ETL::Transform::DefaultTransform.new(flexmock('control'), 
          nil, {:default_value => 'foo'})
